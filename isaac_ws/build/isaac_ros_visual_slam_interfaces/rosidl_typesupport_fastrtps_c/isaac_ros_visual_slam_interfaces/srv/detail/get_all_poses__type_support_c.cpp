  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = isaac_ros_visual_slam_interfaces__srv__GetAllPoses_Request;
    is_plain =
      (
      offsetof(DataType, max_count) +
      last_member_size
      ) == ret_val;
  }
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = isaac_ros_visual_slam_interfaces__srv__GetAllPoses_Response;
    is_plain =
      (
